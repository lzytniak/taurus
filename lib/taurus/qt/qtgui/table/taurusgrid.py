...
#            table.setRowHeight(row,5+25*sum(len(dct[self.rows[row]][col]) for col in self.columns))
        ...
        for i in range(len(self.rows)):
            ...
            if layout_col == 3:
                layout_col = 0
                layout_row += 1
            QtCore.QObject.connect(checkbox, QtCore.SIGNAL(
                'toggled(bool)'), self.show_hide_rows)
        self.show_hide_rows()
        ...
        for i in range(len(self.columns)):
            ...
            if layout_col == 3:
                ...
                layout_row += 1
            QtCore.QObject.connect(checkbox, QtCore.SIGNAL(
                'toggled(bool)'), self.show_hide_columns)
        ...
        for row in values:
            ...
            for cell in row:
                ...
                for synoptic in sorted(cell):
                    ...
                    QtCore.QObject.connect(synoptic_value, QtCore.SIGNAL(
                        "itemClicked(QString)"), self.itemClicked)
                ...
                QtCore.QObject.connect(cell_frame, QtCore.SIGNAL(
                    "itemClicked(QString)"), self.itemClicked)
